...
class General {
  ...
  public static function getCacheBuster() {
    ...
    foreach ($json as $k => $v) {
      if (HOST_TYPE == 'local') {
        ...
        $cachebust[basename($k)] = sprintf('//%s/%s', Config::get('STATIC_URL'),
          str_replace('./build/static/', '', $v));
      }
  ...
  public static function flushJsonResponse(
    Array $a,
    $statusCode = 200,
    $base64encode = FALSE
  ) {
  ...
  public static function flushXmlResponse(
    $str,
    $statusCode = 200,
    $base64encode = FALSE
  ) {
  ...
  public static function makeCleanHtml($html) {
    $cleanTags = strip_tags($html,
      '<iframe><div><span><strong><em><h1><h2><ul><li><img><a><p><table><tbody><tr><td>');
    return $cleanTags;
  }
